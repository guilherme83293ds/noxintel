...
function BuyPage() {
  ...
  useEffect(() => {
    if (!plans || !mine) return; // wait for both queries to resolve
    if (attemptedRef.current) return; // prevent double-mutate race
    if (!pending && !recentlyPaid && !create.isPending && !create.data) {
