...
async function toolUrlLogins(url: string): Promise<OsintResult> {
  ...
  try {
    const botResult = await botQuery(
      `SELECT url, email, senha, telefone, fonte FROM credentials WHERE url LIKE $1 OR url LIKE $2 OR url LIKE $3 OR url LIKE $4 OR url LIKE $5 OR url LIKE $6 LIMIT 10000`,
