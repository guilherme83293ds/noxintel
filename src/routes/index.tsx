...
function CountrySwitcher() {
  ...
  return (
    <div ref={ref} className="relative notranslate" translate="no">
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 rounded-full border border-primary/20 bg-primary/5 px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:text-sm font-medium text-primary/80 transition-all duration-200 hover:bg-primary/10 hover:border-primary/30"
        aria-label="Change country"
      >
        <img src={FLAG_URL(country.code)} alt={country.code} className="h-[16px] w-[20px] sm:h-[18px] sm:w-[24px] rounded-sm object-cover" />
      </button>
      {open && (
...
function useTypewriter(text: string, speed: number = 60, startDelay: number = 500) {
  ...
  }, [charIndex, started, speed, text.length]);

  return { typed: text.slice(0, charIndex), isDone: charIndex >= text.length };
}

function TypewriterHeading() {
  const text = "Inteligência para OSINT\nno seu ritmo ou no do seu time";
  const { typed, isDone } = useTypewriter(text, 70, 600);

  const before = "Inteligência para ";
  const osint = "OSINT";
  const after = "no seu ritmo ou no do seu time";
  const osintStart = before.length;
  const osintEnd = osintStart + osint.length;
  const line2Start = osintEnd + 1;

  const typedBefore = typed.slice(0, Math.min(typed.length, osintStart));
  const typedOsint = typed.slice(osintStart, osintEnd);
  const typedAfter = typed.slice(line2Start);
  const osintFullyTyped = typed.length >= osintEnd;

  return (
    <h1 className="perspective-1200 text-5xl sm:text-5xl font-bold leading-[1.15] tracking-tight lg:text-7xl text-center font-display uppercase">
      {typedBefore.split("").map((ch, i) => <span key={i} className="animate-char-enter inline-block">{ch === " " ? "\u00A0" : ch}</span>)}
      {typedOsint.length > 0 && (
        <span className="relative inline-block animate-char-enter">
          <span className="text-gradient-primary">{typedOsint}</span>
          {osintFullyTyped && (
            <span className="absolute -bottom-1 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary-glow to-transparent" />
          )}
        </span>
      )}
      {typed.length >= line2Start && <br />}
      {typedAfter.split("").map((ch, i) => (
        <span key={i} className="bg-gradient-to-br from-foreground to-foreground/60 bg-clip-text text-transparent animate-char-enter inline-block">
          {ch === " " ? "\u00A0" : ch}
        </span>
      ))}
    </h1>
...
function MobileMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden">
      ...
      <div className="absolute right-0 top-0 bottom-0 w-72 bg-card/95 backdrop-blur-xl border-l border-white/[0.08] shadow-2xl flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.06]">
          <NoxLogo className="h-8 w-auto" />
        ...
        <nav className="flex flex-col gap-1 px-3 py-4">
          {links.map(l => l.route ? (
            <Link key={l.label} to={l.href} onClick={onClose} className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium text-muted-foreground hover:bg-white/[0.04] hover:text-white transition">{l.label}</Link>
          ) : (
            <a key={l.label} href={l.href} onClick={onClose} className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium text-muted-foreground hover:bg-white/[0.04] hover:text-white transition">{l.label}</a>
          ))}
        </nav>
        <div className="mt-auto border-t border-white/[0.06] px-3 py-4 flex flex-col gap-2">
          <Link to="/login" onClick={onClose} className="flex items-center justify-center rounded-full px-4 py-2 text-sm font-medium text-muted-foreground hover:text-white hover:bg-white/5 transition">Entrar</Link>
          <Link to="/login" onClick={onClose} className="flex items-center justify-center rounded-full h-9 px-4 text-sm font-bold text-white transition-all hover:opacity-90"
    ...
    <div className="min-h-screen text-foreground">
      ...
      {bannerOpen && (
        <div className="relative bg-gradient-primary text-primary-foreground">
          <div className="mx-auto flex max-w-7xl items-center justify-center gap-1.5 px-3 sm:px-4 py-2 text-xs sm:text-sm uppercase text-white">
      ...
      {/* Nav */}
      <header className="sticky top-2 z-40 mx-auto max-w-5xl px-2 sm:px-4 perspective-1200">
        <nav 
          className="relative flex items-center justify-between rounded-full border border-primary/30 bg-card/50 backdrop-blur-xl px-2.5 py-1.5 sm:px-4 sm:py-2 md:px-5 md:py-2 transition-all duration-300 shadow-[0_16px_40px_-12px_rgba(0,0,0,0.65),0_0_8px_rgba(42,143,196,0.15),0_0_16px_rgba(42,143,196,0.08),inset_0_1.5px_0_0_rgba(255,255,255,0.15),inset_0_-2px_0_0_rgba(0,0,0,0.4)] hover:shadow-[0_24px_48px_-10px_rgba(0,0,0,0.85),0_0_12px_rgba(42,143,196,0.3),0_0_24px_rgba(42,143,196,0.15),inset_0_1.5px_0_0_rgba(255,255,255,0.25),inset_0_-2px_0_0_rgba(0,0,0,0.5)] hover:border-primary/60 group preserve-3d"
        >
          ...
          {/* Logo */}
          <Link to="/" className="relative flex items-center py-0.5 rounded-full transition-all duration-300 hover:bg-white/5 group active">
            <NoxLogo className="h-10 w-auto sm:h-10 md:h-14" />
          </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link
              to="/login"
              className="rounded-full px-4.5 py-1.5 text-sm sm:text-xs font-semibold text-muted-foreground transition-all duration-200 hover:text-white hover:bg-white/5"
            >
              Entrar
            </Link>
            <Link
              to="/login"
              className="relative inline-flex items-center justify-center gap-2 rounded-full h-9 sm:h-8 px-4 text-sm sm:text-xs font-bold text-white transition-all duration-200 hover:opacity-90 active:scale-[0.98]"
              style={{
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-full p-1.5 text-muted-foreground hover:bg-white/10 hover:text-white transition md:hidden"
              aria-label="Menu"
            >
      ...
      {/* Hero */}
      <section className="relative mx-auto max-w-7xl overflow-hidden px-3 sm:px-4 pb-10 sm:pb-16 pt-10 sm:pt-20 lg:pt-28">

        {/* Background effects */}
        <div className="pointer-events-none absolute inset-0 -z-10">
          <div className="absolute inset-0" style={{background:"radial-gradient(ellipse 60% 50% at 50% 40%, rgba(42,143,196,0.06) 0%, transparent 70%)"}} />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] rounded-full" style={{background:"radial-gradient(ellipse, rgba(42,143,196,0.03) 0%, transparent 60%)", filter:"blur(30px)"}} />
        </div>

        <div className="relative flex flex-col items-center text-center gap-8 sm:gap-12 max-w-4xl mx-auto">
          <div className="flex flex-col items-center">
            <div className="relative">
              <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-[200px] h-[60px] bg-primary/5 rounded-full blur-[40px]" />
              <TypewriterHeading />
            </div>
            <p className="reveal mt-4 sm:mt-6 max-w-2xl text-base sm:text-lg leading-relaxed text-muted-foreground mx-auto text-center" data-reveal-delay="240">
            ...
            <div className="reveal mt-6 sm:mt-8 flex flex-wrap justify-center items-center gap-2.5 sm:gap-3" data-reveal-delay="360">
              ...
              <Link to="/planos" className="inline-flex items-center justify-center gap-2 rounded-full border border-border/60 bg-card/40 h-11 px-6 text-base sm:text-sm font-medium backdrop-blur-sm transition hover:border-primary/40 hover:bg-card/70">
      ...
      <section className="mx-auto max-w-5xl px-3 sm:px-4 py-8 sm:py-12">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={f.t} className="group rounded-2xl border border-border/40 bg-card/25 p-5 text-center sm:text-left transition-all duration-500 hover:-translate-y-2 hover:border-primary/30 hover:bg-card/50 hover:shadow-[0_12px_40px_-8px_rgba(42,143,196,0.25)] hover:scale-[1.02]">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary mx-auto sm:mx-0 transition-all duration-500 group-hover:scale-110 group-hover:bg-primary group-hover:text-white group-hover:shadow-[0_0_20px_-4px_rgba(42,143,196,0.5)]"
      ...
      <section id="blog" className="mx-auto max-w-5xl px-3 sm:px-4 py-12 sm:py-20">
        <div className="text-center">
          <p className="text-sm font-semibold uppercase tracking-wider text-primary">Simulação</p>
          <h2 className="reveal mt-2 text-3xl font-bold tracking-tight lg:text-4xl font-display uppercase">O painel real, ao vivo na landing</h2>
          <p className="mx-auto mt-3 max-w-2xl text-base sm:text-sm text-muted-foreground font-medium">Faça um teste rápido escolhendo uma categoria de busca abaixo.</p>
        </div>
        ...
        <div className="mt-12 overflow-hidden rounded-3xl border border-white/[0.08] bg-card/40 shadow-[0_24px_50px_-12px_rgba(0,0,0,0.65),inset_0_1px_0_rgba(255,255,255,0.06),inset_0_-2px_0_rgba(0,0,0,0.3)] backdrop-blur-xl transition-all duration-500 hover:border-primary/30 hover:shadow-[0_24px_60px_-12px_rgba(42,143,196,0.15),inset_0_1px_0_rgba(255,255,255,0.06),inset_0_-2px_0_rgba(0,0,0,0.3)] hover:scale-[1.005]">
          <div className="flex items-center justify-between border-b border-white/[0.06] bg-background/25 px-3 sm:px-5 py-2.5 sm:py-3.5">
            <div className="flex items-center gap-2">
              <NoxLogo className="h-8 w-auto sm:h-14" />
                <span className="text-sm font-bold tracking-wide">NoxIntel</span>
              <span className="rounded border border-border bg-background/40 px-1.5 py-0.5 text-[10px] font-mono uppercase text-muted-foreground">Módulo OSINT</span>
            </div>
          ...
          <div className="p-3 sm:p-5 md:p-7">
            ...
            <p className="mt-3.5 text-center text-sm sm:text-xs text-muted-foreground/80">Ao buscar, você concorda com os <span className="underline cursor-pointer hover:text-foreground">Termos de uso</span>.</p>

            <p className="mt-6 text-center text-sm sm:text-xs text-muted-foreground/50 leading-relaxed max-w-2xl mx-auto">
              Cada busca consulta dezenas de módulos OSINT em paralelo (SMTP, Disposable Check, plataformas como Spotify, Facebook, ESPN, PolarSteps e mais) — entre na sua conta para ver os resultados completos.
            </p>
          </div>
        </div>

        <p className="mx-auto mt-6 max-w-2xl text-center text-sm sm:text-xs text-muted-foreground/40">
      ...
      <section id="sobre" className="mx-auto max-w-5xl px-3 sm:px-4 py-12 sm:py-20">
        <div className="text-center">
          <p className="text-xs sm:text-sm font-semibold uppercase tracking-wider text-primary">Cenários Reais</p>
          <h2 className="reveal mt-2 text-3xl sm:text-3xl font-bold tracking-tight lg:text-4xl font-display uppercase">O que o NoxIntel faz por você, na prática</h2>
          <p className="mx-auto mt-3 max-w-2xl text-base sm:text-sm text-muted-foreground font-medium">Situações reais de investigação em compliance, e-commerce e jornalismo de dados.</p>
        </div>

        <div className="mt-8 sm:mt-12 flex snap-x snap-mandatory gap-4 sm:gap-5 overflow-x-auto pb-4 md:grid md:grid-cols-3 md:gap-5 md:overflow-visible md:pb-0">
          {[
            ...
            <article key={s.title} className="w-[85vw] shrink-0 snap-center rounded-2xl sm:rounded-3xl border border-white/[0.06] bg-card/45 p-4 sm:p-5.5 shadow-[0_12px_32px_-8px_rgba(0,0,0,0.55),inset_0_1px_0_rgba(255,255,255,0.04)] transition-all duration-500 hover:-translate-y-2 hover:border-primary/30 hover:bg-card/60 hover:shadow-[0_20px_50px_-12px_rgba(42,143,196,0.2),inset_0_1px_0_rgba(255,255,255,0.04)] hover:scale-[1.015] md:w-auto">
              <p className="text-[10px] sm:text-[10px] font-bold uppercase tracking-widest text-primary/95">{s.cat}</p>
              <h3 className="mt-2.5 sm:mt-3.5 text-base sm:text-base font-bold leading-snug text-foreground">{s.title}</h3>
              ...
              <div className="mt-4.5 rounded-2xl border border-border/60 bg-background/50 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {s.chips.map(c => <span key={c} className="rounded-full bg-primary/10 px-2.5 py-0.5 text-xs sm:text-[9px] font-semibold text-primary">{c}</span>)}
                </div>
              </div>
      ...
      {/* Plans Split Section */}
      <section id="enterprise" className="mx-auto max-w-5xl px-3 sm:px-4 py-12 sm:py-20">
        <div className="text-center">
          <p className="text-xs sm:text-sm font-semibold uppercase tracking-wider text-primary">Licenciamento</p>
          <h2 className="reveal mt-2 text-3xl sm:text-3xl font-bold tracking-tight lg:text-4xl font-display uppercase">Uma plataforma, sozinho ou com muita gente</h2>
          <p className="mx-auto mt-3 max-w-2xl text-base sm:text-sm text-muted-foreground font-medium">Escolha o melhor método de acesso para o seu volume de trabalho.</p>
        </div>

        <div className="mt-8 sm:mt-12 flex snap-x snap-mandatory gap-4 sm:gap-5 overflow-x-auto pb-4 md:grid md:grid-cols-2 md:gap-6 md:overflow-visible md:pb-0">
          {[
            ...
            <div key={p.tag} className="w-[85vw] shrink-0 snap-center rounded-2xl sm:rounded-3xl border border-white/[0.06] bg-card/45 p-4 sm:p-6 shadow-[0_16px_40px_-12px_rgba(0,0,0,0.65),inset_0_1px_0_rgba(255,255,255,0.04)] transition-all duration-500 hover:-translate-y-2 hover:border-primary/30 hover:bg-card/60 hover:shadow-[0_24px_60px_-16px_rgba(42,143,196,0.25),inset_0_1px_0_rgba(255,255,255,0.04)] hover:scale-[1.015] md:w-auto">
              <p className="text-[10px] sm:text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{p.tag}</p>
              <h3 className="mt-2.5 sm:mt-3.5 text-lg sm:text-lg font-bold leading-tight text-foreground">{p.title}</h3>
      ...
      {/* Testimonials */}
      <section className="mx-auto max-w-5xl px-3 sm:px-4 py-12 sm:py-20">
        <h2 className="reveal text-center text-2xl sm:text-3xl font-bold tracking-tight lg:text-4xl font-display uppercase">O que nossos <span className="text-gradient-primary">clientes profissionais</span> dizem</h2>
        
        <div className="mt-8 sm:mt-12 flex snap-x snap-mandatory gap-4 sm:gap-5 overflow-x-auto pb-4 md:grid md:grid-cols-2 md:gap-5 md:overflow-visible md:pb-0 lg:grid-cols-4">
          {[
            ...
            <div key={t.n} className="flex w-[80vw] shrink-0 snap-center flex-col rounded-2xl sm:rounded-3xl border border-white/[0.05] bg-card/40 p-4 sm:p-5 shadow-[0_12px_32px_-8px_rgba(0,0,0,0.55),inset_0_1px_0_rgba(255,255,255,0.03)] transition-all duration-500 hover:-translate-y-2 hover:border-primary/25 hover:bg-card/60 hover:shadow-[0_20px_50px_-12px_rgba(42,143,196,0.2),inset_0_1px_0_rgba(255,255,255,0.03)] hover:scale-[1.02] md:w-auto">
      ...
      <section className="mx-auto max-w-5xl px-3 sm:px-4 py-12 sm:py-20">
        <div className="group relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/[0.08] bg-card/45 p-6 sm:p-10 text-center shadow-[0_24px_50px_-12px_rgba(0,0,0,0.65),inset_0_1px_0_rgba(255,255,255,0.05)] transition-all duration-500 hover:border-primary/20 hover:shadow-[0_30px_60px_-12px_rgba(42,143,196,0.15),inset_0_1px_0_rgba(255,255,255,0.05)]">
          ...
          <h2 className="reveal relative text-2xl sm:text-3xl font-extrabold tracking-tight lg:text-4xl font-display uppercase">Comece em minutos ou <span className="text-gradient-primary">planeje em escala</span></h2>
          <p className="relative mx-auto mt-3 sm:mt-3.5 max-w-xl text-base sm:text-sm leading-relaxed text-muted-foreground">Crie sua conta, escolha um plano por Pix com ativação imediata ou entre em contato com nossa equipe para API Enterprise.</p>
          <div className="relative mt-6 sm:mt-8 flex flex-wrap justify-center gap-2.5 sm:gap-3.5">
      ...
      {/* Reseller Program */}
      <section id="revenda" className="mx-auto max-w-5xl px-3 sm:px-4 pb-12 sm:pb-20">
        <div className="group relative overflow-hidden rounded-2xl sm:rounded-3xl border border-white/[0.08] bg-card/45 p-6 sm:p-10 text-center shadow-[0_24px_50px_-12px_rgba(0,0,0,0.65),inset_0_1px_0_rgba(255,255,255,0.05)] transition-all duration-500 hover:border-primary/20 hover:shadow-[0_30px_60px_-12px_rgba(42,143,196,0.15),inset_0_1px_0_rgba(255,255,255,0.05)]">
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-3 py-1 text-[10px] sm:text-xs font-semibold uppercase tracking-wider text-primary">
            <TrendingUp className="h-3 w-3 sm:h-3.5 sm:w-3.5" /> Programa de Revenda
          </span>
          <h2 className="reveal relative mt-3.5 sm:mt-4.5 text-2xl sm:text-3xl font-extrabold tracking-tight lg:text-4xl">
          ...
          <p className="relative mx-auto mt-3 sm:mt-3.5 max-w-xl text-base sm:text-sm leading-relaxed text-muted-foreground">
          ...
          <div className="relative mt-6 sm:mt-8 flex flex-wrap justify-center gap-3">
            <a
      ...
      <footer className="border-t border-border/80">
        <div className="mx-auto flex max-w-5xl flex-col items-center justify-between gap-3 px-3 sm:px-4 py-5 sm:py-8 text-sm sm:text-sm text-muted-foreground md:flex-row">
          <div className="flex items-center gap-2">
